...
(function () {
	...
	var isInteger = function (literal) {
		...
		if (typeof literal === "string" && regInteger.test(literal)) {
			return true;
		}
		return false;
	};
	...
	Set.prototype = Object.create(Array.prototype, {
		...
		isFinite: { value: () => this.finite },  // TODO: Check if refactoring ok
		isConcrete: { value: () => this.concrete },  // TODO: Check if refactoring ok
		card: {
			value: function () {
				if (this.isConcrete()) {
	...
	SetExtension.prototype = Object.create(Set.prototype, {
		...
		toString: {
			value: function () {
				var toStringArray = Array.prototype.map.call(this, e =>
					typeof e === "undefined" ? e : e.toString()
				);
	...
	var min = function (S) {
		...
		if (S instanceof Set && S.length > 0) {
			S = S.map(e => Integer(e));
			if (S.every(e => e instanceof Integer)) {
				result = S[0];
	...
	TotalSurjectiveRelations.prototype = Object.create(TotalRelations.prototype, {
		...
		toLiteral: {
			value: () => prefix + ".TotalSurjectiveRelations(" + this.S.toLiteral() + "," + this.T.toLiteral() + ")",  // TODO: Check if refactoring ok
			writable: true
		},

		toString: {
			value: () => this.S.toString() + " \ue102 " + this.T.toString(), // TODO: Check if refactoring ok
			writable: true
		},
	...
	var domainRestriction = function (S, r) {
		S = S.concrete ? S : S.toSetExtension();
		return SetExtension.apply(null, r.filter(pair => S.contains(pair.left))); // TODO: Check if refactoring ok
	};
	...
	var rangeRestriction = function (r, T) {
		...
		return SetExtension.apply(null, r.filter(pair => T.contains(pair.right))); // TODO: Check if refactoring ok
	};
	...
	var backwardComposition = function ( /* relationList */) {
		return forwardComposition.apply(
			null, 
			[].slice.call(arguments, 0).revers()
		); // TODO: Check if refactoring ok
	};
	...
	var override = function ( /* relationList */) {
		...
		relationListArray = relationListArray.map(s => s.concrete ? s : s.toSetExtension());
	...
	PartialFunctions.prototype = Object.create(Relations.prototype, {
		...
		toLiteral: {
			value: () => prefix + ".PartialFunctions(" + this.S.toLiteral() + "," + this.T.toLiteral() + ")", // TODO: Check if refactoring ok
			writable: true
		},
	...
	TotalFunctions.prototype = Object.create(PartialFunctions.prototype, {
		...
		anyMember: {
			value: () => this.makeSetExtension(e => this.T.anyMember()), // TODO: Check if refactoring ok
			writable: true
		},

		toLiteral: {
			value: () => prefix + ".TotalFunctions(" + this.S.toLiteral() + "," + this.T.toLiteral() + ")", // TODO: Check if refactoring ok
			writable: true
		},
	...
	PartialInjections.prototype = Object.create(PartialFunctions.prototype, {
		...
		toLiteral: {
			value: () => prefix + ".PartialInjections(" + this.S.toLiteral() + "," + this.T.toLiteral() + ")", // TODO: Check if refactoring ok
			writable: true
		},
	...
	TotalInjections.prototype = Object.create(TotalFunctions.prototype, {
		...
		toLiteral: {
			value: () => prefix + ".TotalInjections(" + this.S.toLiteral() + "," + this.T.toLiteral() + ")", // TODO: Check if refactoring ok
			writable: true
		},
	...
	var functionImage = function (f, E) {
		...
		if (!f.functionImage) { // case defined fImage
			return;
		}
	...
	Array.prototype.toString = function () {
		var toStringArray = this.map(e => typeof e === "undefined" ? e : e.toString());
	...
	jeb.util.clone = function (obj) {
		...
		if (typeof obj === "string") { // case string
			return obj;
		}
		
		if (obj instanceof Set && obj.isConcrete()) {
	...
	jeb.util.extend($B, {
		...
		becomesEqualTo,
		becomesMemberOf,
		becomesSuchThat /* before JeB_08_16 , */
	});
